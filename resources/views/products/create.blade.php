            ...
            <div class="text-center mb-3">
                <div class="d-inline-flex align-items-center bg-white rounded-pill shadow-sm px-3 py-2 mb-2">
                    ...
                    <div class="text-start">
                        <h4 class="mb-0 fw-bold text-dark">إضافة منتج جديد</h4>
                        <p class="text-muted mb-0 small">أدخل تفاصيل المنتج والوحدات والأسعار</p>
                    </div>
                </div>
            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <div id="imagePreview" class="mt-2 d-none">
                                    <div class="border border-dashed rounded p-2 bg-light text-center">
                                        <img src="#" alt="معاينة الصورة" class="img-fluid rounded" style="max-height: 100px">
                            ...
                            <div class="col-12">
                                <div class="card bg-light border">
                                    <div class="card-body p-2">
                                        ...
                                        <div id="serial_number_container" class="{{ old('has_serial') ? '' : 'd-none' }}">
                                            <input type="text" 
                                    ...
                                    <div class="card-body p-3 pt-4">
                                        <div class="row g-3">
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                <div class="input-group">
                                                    <select class="form-select" name="units[0][unit_id]" required>
                                                        <option value="">اختر الوحدة</option>
                                                        @foreach($units as $unit)
                                                        <option value="{{ $unit->id }}" 
                                                                {{ old('units.0.unit_id') == $unit->id ? 'selected' : '' }}>
                                                            {{ $unit->name }}
                                                        </option>
                                                        @endforeach
                                                    </select>
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                <div class="barcode-container">
                                                    <div class="barcode-entries">
                                                        <!-- Initial barcode entry -->
                                                        <div class="barcode-entry d-flex align-items-center mb-2">
                                                            <input type="text" class="form-control form-control-sm" name="units[0][barcodes][]" placeholder="أدخل الباركود">
                                        ...
                                        <div class="mt-3">
                                            ...
                                            <div class="row prices-row g-2">
                                                @php $defaultPriceType = $priceTypes->first(function($pt) { return $pt->is_default; }); @endphp
                                                @if($defaultPriceType)
                                                <div class="col-md-6 price-type-col">
                                                    <div class="card h-100 border-success border-opacity-25">
                                                        <div class="card-body p-2">
                                                            <label class="form-label fw-semibold small text-muted mb-1">
                                                                {{ $defaultPriceType->name }}
                                                            </label>
                                                            <div class="input-group input-group-sm">
                                                                ...
                                                                <input type="number" 
                                                                       class="form-control border-success border-opacity-50"
                                                                       name="units[0][prices][{{ $defaultPriceType->id }}][value]"
                                                                       value="{{ old('units.0.prices.' . $defaultPriceType->id . '.value') }}"
                                                                       step="0.01" 
                                                                       min="0" 
                                                                       required
                                                                       placeholder="0.00">
                                                                <input type="hidden" 
                                                                       name="units[0][prices][{{ $defaultPriceType->id }}][price_type_id]" 
                                                                       value="{{ $defaultPriceType->id }}">
                                                            </div>
                                                            @error('units.0.prices.' . $defaultPriceType->id . '.value')
                                                            <div class="invalid-feedback">{{ $message }}</div>
                                                            @enderror
                                                        </div>
                                                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // When a new UNIT is added
    document.getElementById('add-unit').addEventListener('click', function() {
        // The existing clone logic in the blade file will create a new unit row
        // We just need to clean up the barcodes section in the new clone
        setTimeout(() => {
            const allUnitRows = document.querySelectorAll('.unit-row');
            const newUnitRow = allUnitRows[allUnitRows.length - 1];
            const barcodeContainer = newUnitRow.querySelector('.barcode-container');
            const barcodeEntries = barcodeContainer.querySelector('.barcode-entries');
            const newIndex = allUnitRows.length - 1;

            // 1. Remove all but the first barcode entry
            while (barcodeEntries.children.length > 1) {
                barcodeEntries.removeChild(barcodeEntries.lastChild);
            }
            
            // 2. Clear the value of the remaining input and update its name
            const firstInput = barcodeEntries.querySelector('input[type="text"]');
            if (firstInput) {
                firstInput.value = '';
                firstInput.name = `units[${newIndex}][barcodes][]`;
            }
            
        }, 100); // Delay to ensure clone is in DOM
    });
    ...
    document.getElementById('add-unit').addEventListener('click', function() {
                    ...
                    <div class="card-body p-2">
                        ...
                        <div class="input-group input-group-sm">
                            ...
                            <input type="number" class="form-control"
                                ...
                                step="0.01" min="0" ${priceType.is_default ? 'required' : ''} placeholder="0.00">
        ...
        // إعادة تعيين مستمعي الأحداث للباركود
        const generateBtn = template.querySelector('.generate-unit-barcode');
        if (generateBtn) {
            generateBtn.addEventListener('click', generateUnitBarcode);
        }
        // إذا كان هناك باركود للوحدة الرئيسية، قم بتوليد باركود للوحدة الجديدة
        const firstUnitInput = document.querySelector('.unit-barcode');
        const mainBarcode = firstUnitInput ? firstUnitInput.value : '';
        if (mainBarcode) {
            const unitBarcodeInput = template.querySelector('.unit-barcode');
            if (unitBarcodeInput) {
                if (unitIndex === 0) {
                    unitBarcodeInput.value = mainBarcode;
                } else {
                    unitBarcodeInput.value = mainBarcode + String.fromCharCode(65 + unitIndex - 1);
                }
            }
        }
        // Setup price type buttons for the new unit
        setupPriceTypeButtons();
        unitIndex++;
    });

    // توليد باركود الوحدة
    function generateUnitBarcode() {
        const unitInputs = document.querySelectorAll('.unit-barcode');
        if (unitInputs.length === 0) return;
        
        const inputGroup = this.closest('.input-group');
        if (!inputGroup) return;
        
        const currentInput = inputGroup.querySelector('.unit-barcode');
        if (!currentInput) return;
        
        const index = Array.from(unitInputs).indexOf(currentInput);
        
        // Generate main barcode if clicking first unit
        if (index === 0) {
            const newMain = Math.floor(Math.random() * 9000000000) + 1000000000;
            // Set main unit barcode
            if (unitInputs[0]) {
                unitInputs[0].value = newMain;
            }
            // Propagate to other units
            unitInputs.forEach((input, i) => {
                if (i > 0 && input) {
                    input.value = newMain + String.fromCharCode(65 + i - 1);
                }
            });
        } else {
            // Use existing main barcode to generate for this unit
            const mainBarcode = unitInputs[0] ? unitInputs[0].value : '';
            if (!mainBarcode) {
                alert('يجب توليد الباركود للوحدة الرئيسية أولاً');
                return;
            }
            currentInput.value = mainBarcode + String.fromCharCode(65 + index - 1);
        }
    }

    // إضافة مستمع الحدث للزر الأول إذا وُجد
    const generateUnitBarcodeBtn = document.querySelector('.generate-unit-barcode');
    if (generateUnitBarcodeBtn) {
        generateUnitBarcodeBtn.addEventListener('click', generateUnitBarcode);
    }
    ...
    // Remove unused Tagify code as it's not being used in the current barcode implementation
});
</script>
@endpush
@endsection 