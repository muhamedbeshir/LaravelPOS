        ...
        <div class="col-md-8">
            <div class="card shadow-sm">
                <div class="card-header">
                    <h4 class="mb-0"><i class="fas fa-plus-circle me-2"></i>{{ __('إضافة إيداع جديد') }}</h4>
                </div>
                ...
                <div class="card-body">
                    <form action="{{ route('deposits.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('deposit_source_id') is-invalid @enderror" id="deposit_source_id" name="deposit_source_id" required>
                                ...
                                @foreach($sources as $id => $name)
                                    <option value="{{ $id }}" {{ old('deposit_source_id') == $id ? 'selected' : '' }}>{{ $name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="d-flex justify-content-end">
                            ...
                            <button type="submit" class="btn btn-success">{{ __('حفظ الإيداع') }}</button>
                        </div>
                    </form>
                </div>
            </div>
